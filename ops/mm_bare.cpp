...
tt_mm_bare_op::tt_mm_bare_op(
    ...
    bool is_int32_matmul = intermed_data_format == DataFormat::Int32;
    for (int i=0;i<in_data_format.size();i++) {
        if (in_data_format[i] == DataFormat::Int32) {
            is_int32_matmul = true;
        }
    }
    ...
    set_hlk_cpp_file_name_all_cores("hlks/" + get_hlks_file_name_no_extension(identity, is_int32_matmul) + ".cpp");
    ...
    if (identity) {
        ...
        max_num_inputs =
            is_int32_matmul ? matmul_u_int32::MATMUL_U_INT32_MAX_NUM_INPUTS : matmul_u::MATMUL_U_MAX_NUM_INPUTS;
    }
