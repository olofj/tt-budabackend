...
namespace pipegen2::PerfInfoInternal
{
    void HostSpillModeAttributes::set_attributes(uint64_t selected_worker_queue_header_addr,
    ...
    void WorkerCore::set_host_spill_mode_attributes(uint64_t selected_worker_queue_header_addr,
                                                    uint64_t host_trace_info,
                                                    uint64_t host_dest_address)
    {
    ...
    uint32_t calculate_worker_mem_size(std::size_t num_workers)
    {
        return num_workers > 0 ?
            (dram_mem::address_map::DRAM_EACH_BANK_PERF_BUFFER_SIZE / num_workers) & 0xFFFFFFE0 : 0;
    }
                                           ...
                                           PerfDumpLevel perf_dump_level)
    {
        for (WorkerCore& worker : workers)
        {
            ...
            ASSERT(worker_num_thread_slots < (0xFFFF - 4), "Request counter wrapped around!");
                                         ...
                                         const SoCInfo* soc_info)
    {
        ...
        const unsigned long host_pcie_buf_addr =
